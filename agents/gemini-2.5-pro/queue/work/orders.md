...
Read and complete the workorder in: ops/wo/wip/haiku-fix_agent_tail_segfault.md
...
FIRST THING you do:
```bash
python bin/next_step ops/wo/wip/haiku-fix_agent_tail_segfault.md START
```

AS YOU WORK, journal before each action:
```bash
python bin/next_step ops/wo/wip/haiku-fix_agent_tail_segfault.md "reading config.py to understand ConfigManager"
# then do the reading

python bin/next_step ops/wo/wip/haiku-fix_agent_tail_segfault.md "implementing enable/disable commands"
# then do the coding

python bin/next_step ops/wo/wip/haiku-fix_agent_tail_segfault.md "writing tests in tests/test_foo.py"
# then write tests
```

WHEN DONE:
```bash
python bin/next_step ops/wo/wip/haiku-fix_agent_tail_segfault.md COMPLETE
```
...
1. Read ops/wo/wip/haiku-fix_agent_tail_segfault.md for your task instructions
...
7. `python bin/next_step ops/wo/wip/haiku-fix_agent_tail_segfault.md COMPLETE`
