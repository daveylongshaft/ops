# Task: Migrate test_backup_service.py to Live Client Integration Test

## Goal
Rewrite the mock-based unit test `tests/test_backup_service.py` to be a live integration test using the CSC client in programmatic mode, communicating with a live server.

## Instructions

1.  **Preparation:**
    *   Identify the core functionality tested in `tests/test_backup_service.py`.
...
2.  **Rewrite the Test:**
    *   Create a new test file: `tests/live_backup_service_test.py`.
...
3.  **Cleanup & Integration:**
    *   Remove the old mock-based test log if it exists: `rm tests/logs/test_backup_service.log`.
    *   The old test file `tests/test_backup_service.py` should remain (or be marked as deprecated) but its log removal forces the system to recognize the need for a fresh run (via the new live test).
    *   Ensure the new test is discovered by the test runner (e.g., `pytest` or `unittest` discovery).

4.  **Git Workflow:**
    *   `git add tests/live_backup_service_test.py`
    *   `git commit -m "Migrate test_backup_service.py to live client integration test"`
    *   `git pull && git push`
...
PID: 470133 agent: gemini-3-pro starting at 2026-02-18 13:05:05

PID: 476198 agent: gemini-2.5-flash starting at 2026-02-18 13:44:54
