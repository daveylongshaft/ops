...
## Objective
Test all methods of the `proof` service module via the AI command interface in #general.
...
## Procedure
For each method:
1. Send: echo "AI 1 proof help" >> /opt/csc/tmp/csc/run/client.in (get current help output)
2. Send: echo "AI 1 proof <method>" >> /opt/csc/tmp/csc/run/client.in
3. Wait 5 seconds, check response in channel or server log:
   journalctl --user -u csc-server.service -n 20 --no-pager | grep -E "Help for proof|<method>|Error"
